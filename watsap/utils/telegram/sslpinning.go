...
func SSLPinning() {
	...
	if err != nil {
		config.Logger("Failed to read certificate file", "error")
		os.Exit(1)
	}
	...
	if ok := caCertPool.AppendCertsFromPEM(cert); !ok {
		config.Logger("Failed to append certificate to pool", "error")
		os.Exit(1)
	}
	...
	if err != nil {
		config.Logger("Failed to make request to API", "error")
	}
